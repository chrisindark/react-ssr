"use strict";var _react=_interopRequireDefault(require("react"));var _express=_interopRequireDefault(require("express"));var _reactRouter=require("react-router");var _reactRedux=require("react-redux");var _server=_interopRequireDefault(require("react-dom/server"));var _App=_interopRequireDefault(require("../src/App"));var _store=_interopRequireDefault(require("../src/store"));var _routes=_interopRequireDefault(require("../src/routes"));function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var path=require('path');var fs=require('fs');var request=require('request');var PORT=process.env.PORT||3000;var app=(0,_express.default)();var router=_express.default.Router();var distPath=path.resolve(__dirname,'../build');// main css, js
...
var route=_routes.default.filter(function(r){return(0,_reactRouter.matchPath)(req.baseUrl,{path:r.path,exact:true});});// extract component based on route
...
Promise.all(dataPromise).then(function(){console.log("Rendering ".concat(req.baseUrl," at ").concat(new Date().toString()," on server"));var html=_server.default.renderToString(_react.default.createElement(_reactRedux.Provider,{store:_store.default},_react.default.createElement(_reactRouter.StaticRouter,{location:req.baseUrl,context:context},_react.default.createElement(_App.default,null))));return res.send(// render the template
...
"<script>window.REDUX_DATA = ".concat(JSON.stringify(_store.default.getState()),"</script>")));});});});app.use(router);app.listen(PORT,function(){console.log('listening on '+PORT+'...');});
